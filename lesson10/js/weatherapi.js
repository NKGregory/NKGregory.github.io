  ...
  .then((jsObject) => {
    ...
    //for (let i = 0; i < jsObject.length; i++ ) {
     // if(jsObject.list[i].dt_txt.includes("18:00")) {
        ...
        //.jsObject.list[4/*i*/].dt_text;
        const imagesrc = 'https://openweathermap.org/img/w/' + jsObject.list[4/*i*/].weather[0].icon + '.png';  // note the concatenation
        const desc = jsObject.list[4/*i*/].weather[0].description;  // note how we reference the weather array
        picture.setAttribute('src', imagesrc);  // focus on the setAttribute() method
        picture.setAttribute('alt', desc);
        temp.innerHTML = Math.round(jsObject.list[4/*i*/].main.temp)+"&deg;"+" F";
